...
public class RentalPage implements ActionListener{
	...
	private void initialize() {
		...
		// 환경설정
		db.getConnection();
		tableheads = new String[] {"도서번호","도서명","아이디","이름","전화번호","대여일","반납일","등록일"};
		...
		btn_RentalCondition.setBackground(SystemColor.activeCaption);
		...
		tablemd = new DefaultTableModel(db.getRentalList(cb_Search.getSelectedItem().toString(),""),tableheads);
		...
		JButton btn_Return = new JButton("반납");	// 빌린 도서를 반납합니다
		btn_Return.setToolTipText("선택한 도서를 반납합니다.");
		btn_Return.setBounds(1472, 725, 86, 33);
		btn_Return.setFont(new Font("Tmon몬소리 Black", Font.PLAIN, 12));
		btn_Return.setBackground(SystemColor.window);
		btn_Return.setFocusPainted(false);
		p_Main.add(btn_Return);
	...
	public void refleshTable(String col, String search) {	//테이블을 새로고침하는 메소드
		Object[][] o = db.getRentalList(col, search);
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "반납" :
			...
			for(int i=0; i<cols.length; i++) {
				db.returnBook(model.getValueAt(cols[i], 0).toString());
				cnt++;
			}
			JOptionPane.showMessageDialog(null, cnt + "개의 도서를 반납했습니다.");
			refleshTable(cb_Search.getSelectedItem().toString(), tf_SearchBar.getText());
			System.out.println(cnt + " Books Return");
